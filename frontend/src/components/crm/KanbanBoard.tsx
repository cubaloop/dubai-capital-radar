      ...
      {KANBAN_COLUMNS.map((col) => {
        ...
        return (
          <div
            ...
            className="flex-shrink-0 w-80 bg-white/95 dark:bg-[#28243D] rounded-2xl p-4 border border-slate-300 dark:border-[#3A354C] card-3d shadow-md flex flex-col space-y-3"
          >
            {/* Column Header */}
            <div className="flex items-center justify-between pb-2 border-b border-slate-300 dark:border-[#3A354C]">
              <div className="flex items-center gap-2">
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-extrabold border ${col.badgeColor}`}>
                  {getColumnTitle(col.id)}
                </span>
                <span className="text-xs text-slate-900 dark:text-slate-200 font-mono font-extrabold">({colLeads.length})</span>
              </div>
            </div>
              ...
              {colLeads.map((lead) => {
                ...
                return (
                  <div
                    ...
                    className="p-4 rounded-xl bg-white dark:bg-[#201D34] hover:bg-slate-50 dark:hover:bg-[#28243D] border border-slate-300 dark:border-[#3A354C] hover:border-primary-500 card-3d shadow-md hover:shadow-xl transition-all cursor-pointer space-y-2.5 group"
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="font-extrabold text-sm text-slate-950 dark:text-white group-hover:text-primary-600 transition-colors">
                        {lead.name}
                      </div>
                      {isSent ? (
                        <span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-emerald-900 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-500/20 px-2 py-0.5 rounded-full border border-emerald-300 dark:border-emerald-500/40 font-mono">
                          <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Enviado
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1 text-[10px] font-extrabold text-slate-950 dark:text-slate-100 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full font-mono border border-slate-300 dark:border-slate-700">
                          Pendiente
                        </span>
                      )}
                    </div>

                    <div className="text-xs text-black dark:text-slate-100 font-mono font-extrabold">
                      {lead.phone}
                    </div>

                    {lead.notes && (
                      <div className="text-xs font-bold text-slate-950 dark:text-slate-100 line-clamp-2 bg-slate-100 dark:bg-slate-800/80 p-2 rounded-lg border border-slate-300 dark:border-slate-700">
                        {lead.notes}
                      </div>
                    )}

                    <div className="flex items-center justify-between pt-1 border-t border-slate-200 dark:border-slate-700 text-xs text-slate-950 dark:text-slate-100 font-bold">
                      <span className="truncate max-w-[150px] text-primary-700 dark:text-primary-400 font-mono font-extrabold">
